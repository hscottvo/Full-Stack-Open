...
const Phonebook = () => {
    ...
    const handleSubmit = (event) => {
        ...
        if (persons.some((person) => person.name === newName)) {
            ...
            setPersons(
                persons.concat({ name: newName, number: newPhoneNumber })
            )
            setNewName('')
            setNewPhoneNumber('')
        }
...
const NumNotification = ({ message, type }) => {
    ...
    switch (type) {
        ...
        case 'alreadyDeleted':
            return (
                <div className="error">
                    Information of {message} has already been removed from
                    server
                </div>
            )
        case 'invalidData':
            console.log(message)
            return (
                <div className="error">Person validation failed: {message}</div>
            )
    }
}
        ...
        objectResponse
            .then((returnedObject) => {
                ...
                if (returnedObject != null) {
                    ...
                    if (
                        persons.some(
                            (person) => person.id === returnedObject.id
                        )
                    ) {
                        setPersons(
                            persons.map((person) =>
            ...
            .catch((error) => {
                ...
                messageHandler(error.response.data.error, 'invalidData')
            })
    }
