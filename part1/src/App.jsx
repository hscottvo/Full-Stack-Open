...
const Content = (props) => {
    const listItems = props.rows.map(row => <p>{row.part} {row.exercise}</p>)
    return (
        <>
            {listItems}
        </>
...
const VoteCounts = ({ counts, voteCount }) => {
    console.log(counts)
    let items = Object.entries(counts).map(
        ([key, value]) => (
            <table>
                <thead>
                    <tr key={key}>
...
const MostVotedAnecdote = ({ counts, anecdotes }) => {
    if (Object.keys(counts).length > 0) {
        let index = Object.keys(counts).reduce((a, b) => counts[a] > counts[b] ? a : b)
        ...
        return (
            <>
                <a>{anecdotes[index]}</a>
                <br />
                <a>has {counts[index]} vote{counts[index] > 1 ? "s" : ""}</a>
            </>
        )
    }
    else {
